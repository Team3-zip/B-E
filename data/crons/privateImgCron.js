    ...
    cron.scheduleJob('0 5 0 * * *', async function () {
        ...
        try {
            ...
            const url = await PrivateImg.findAll({
                attributes: ['houseManageNo', 'url1', 'url2', 'url3', 'url4', 'url5', 'fk_pblancNo']
            });
            for (i in url) {

                urlArr.push({ id: url[i]['houseManageNo'], url1: url[i]['url1'], url2: url[i]['url2'], url3: url[i]['url3'], url4: url[i]['url4'], url5: url[i]['url5'], fk_pblancNo: url[i]['fk_pblancNo'] });
                //console.log(urlArr)
            }
